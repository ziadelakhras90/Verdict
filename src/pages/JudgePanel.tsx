...
export default function JudgePanel() {
  ...
  // Reset expiry guard when a new session starts or room leaves session state
  useEffect(() => {
    if (!room) return
  ...
  // Auto-advance once per expired session only
  useEffect(() => {
    ...
    void handleAdvance(currentSession, true)
  }, [isExpired, isJudge, roomId, room?.status, room?.current_session, advancing])

  async function handleAdvance(expectedSession?: number, autoTriggered = false) {
    ...
    setAdvancing(true)
    try {
      const result = await advanceSession(roomId, sessionSnapshot) as {
        idempotent?: boolean
      ...
      if (result?.moved_to === 'verdict') {
        toast.info('انتهت الجلسات — أصدر حكمك')
      } else if (room.current_session < 3) {
        toast.success(`انتقلنا للجلسة ${room.current_session + 1}`)
      }
    } catch (err) {
      if (autoTriggered) {
        lastHandledExpiredSessionRef.current = null
      }
      toast.error(normalizeErrorMessage(err, 'تعذر الانتقال للجلسة التالية'))
    } finally {
      setAdvancing(false)
    }
  }
  ...
  async function handleVerdictSubmit() {
    ...
    try {
      ...
      const feedback = getEdgeMutationFeedback('submit-verdict', result, {
        message: getEdgeMutationFeedback('submit-verdict', result).isDuplicate ? 'تم حفظ الحكم بالفعل' : 'صدر الحكم',
      })
      toast[feedback.variant](feedback.message)
    } catch (err) {
      ...
      <div className="h-screen flex flex-col max-w-2xl mx-auto">
        ...
        <div className="border-t border-gold/20 p-4 space-y-3 bg-judge/5">
          ...
          {canAdvance && (
            <>
              {isExpired && (
                <Card className="border-gold/20 bg-gold/5 p-3 text-center">
                  <p className="text-xs text-gold/80">
                    انتهى وقت الجلسة — جارٍ تثبيت الانتقال مرة واحدة فقط
                  </p>
                </Card>
              )}
              <Button
